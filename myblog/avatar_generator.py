...
class GithubAvatarGenerator:
    ...
    color_pool_rgb = ((170, 205, 102), (159, 255, 84), (209, 206, 0),
                      (255, 255, 0), (47, 107, 85), (47, 255, 173),
                      (0, 173, 205), (8, 101, 139), (180, 180, 238),
                      (106, 106, 255), (155, 211, 255), (204, 50,
                                                         153), (101, 119, 139))

    def _get_avatar_vertex(self):
        '''
        Generate a vertex of which each value is a boolean value.
        This 5*5 vertex denotes the strcture of 5*5 block vertex
        in github avatar
        '''
        # get 5*5 2d array full of False
        avatar_vertex = np.empty(
            (self.block_vertex_dimension, self.block_vertex_dimension),
            dtype=np.bool)
    ...
    def _get_avatar_data(self):
        ...
        avatar_data = np.zeros((self.avatar_width, self.avatar_width, 3),
                               dtype=np.uint8)
        ...
        for i in range(len(avatar_vertex)):
            for j in range(len(avatar_vertex[i])):
                ...
                if is_True:
                    up_left_point = (self.frame_width + i * self.block_width,
                                     self.frame_width + j * self.block_width)
...
if __name__ == '__main__':
    for i in range(25):
        ...
        path = fr'.\blog\myblog\static\myblog\images\avatar\avatar{i}.jpg'
        gen.save_avatar(path)
